...
norm_area <- function(x){
  x <- as.character(x)
  x <- trimws(x)
  # 半角/全角スペース、NBSP、タブ等をすべて除去
  x <- gsub("[[:space:]\u00A0\u3000]+", "", x, perl = TRUE)
  x
}

# --- 浜北地域（擬似エリア）: 5地域の「先頭ブロック(45*0)」だけを合算 ---
# area_name == "hamakita" のときだけ有効（通常の area_name は従来どおり）
is_hamakita <- identical(area_name, "hamakita")
skip_label_check <- FALSE

if (is_hamakita) {
  base_dir <- here::here("data","processed")
  src_areas <- c("aratama","akasa","nakaze","hamana","kitahama")
  file_paths <- file.path(base_dir, paste0(src_areas, "_population_combined.xlsx"))

  missing <- file_paths[!file.exists(file_paths)]
  if (length(missing) > 0) {
    stop("浜北地域: 元ファイルが見つかりません:\n", paste(missing, collapse = "\n"))
  }

  years_list <- lapply(file_paths, function(fp) {
    excel_sheets(fp) %>%
      grep("^[0-9]{4}-04$", ., value = TRUE) %>%
      sub("-04$", "", .) %>%
      as.integer() %>%
      sort()
  })

  years <- Reduce(intersect, years_list)
  if (length(years) == 0) {
    stop("浜北地域: 5ファイルの共通年（YYYY-04）が見つかりません。")
  }

  # 年範囲が揃っていない場合は明示的に落とす（取りこぼし防止）
  if (!all(vapply(years_list, function(y) identical(y, years), logical(1)))) {
    msg <- paste0(
      src_areas, ": ",
      vapply(years_list, function(y) paste(y, collapse=","), character(1)),
      collapse="\n"
    )
    stop("浜北地域: シート年が揃っていません（5ファイルで年範囲が一致する状態にして下さい）:\n", msg)
  }
  ...
  # 5ファイル×全シートを読み込み（各ファイルの先頭ブロックだけ使う）
  sdata_by_src <- lapply(file_paths, function(fp) {
    lapply(sheets, function(sh) read_excel(path = fp, sheet = sh, col_names = TRUE))
  })

  extract_vec <- function(sheets_list, row, col) {
    vapply(sheets_list, function(dat) as.numeric(dat[row, col][[1]]), numeric(1))
  }
  sum_extract <- function(row, col) {
    Reduce(`+`, lapply(sdata_by_src, extract_vec, row = row, col = col))
  }

  # --- 5歳階級（先頭ブロック=45*0固定） ---
  p0004 <- sum_extract(2, 2);   p0509 <- sum_extract(8, 2);   p1014 <- sum_extract(14, 2)
  p1519 <- sum_extract(20, 2);  p2024 <- sum_extract(26, 2);  p2529 <- sum_extract(32, 2)
  p3034 <- sum_extract(38, 2)

  p3539 <- sum_extract(2, 6);   p4044 <- sum_extract(8, 6);   p4549 <- sum_extract(14, 6)
  p5054 <- sum_extract(20, 6);  p5559 <- sum_extract(26, 6);  p6064 <- sum_extract(32, 6)
  p6569 <- sum_extract(38, 6)

  p7074 <- sum_extract(2, 10);  p7579 <- sum_extract(8, 10);  p8084 <- sum_extract(14, 10)
  p8589 <- sum_extract(20, 10); p9094 <- sum_extract(26, 10); p9500 <- sum_extract(32, 10)

  # --- 集計行（0-14 / 15-64 / 65+ / 総人口）も「元シートの集計値」を合算 ---
  p0014  <- sum_extract(44, 2)
  p1564  <- sum_extract(44, 6)
  p6500  <- sum_extract(44, 10)
  pTotal <- sum_extract(43, 10)
  ...
  # 浜北地域は「合算値」なので、ラベル整合性チェック（check_labels）はスキップする
  skip_label_check <- TRUE


  # ---- 浜北地域: ラベル検査は代表1ファイルで実施（フォーマット確認目的） ----
  # ※ check_labels は「生データ(rawdata)」で回す想定（colNames=FALSE）
  #    浜北は合算列なので NAMES_area/index_vec が一致しないため、代表ファイルの地区一覧で検査します。
  rep_area <- "akasa"  # 代表（必要なら変更可）
  rep_fp <- file_paths[which(src_areas == rep_area)]
  rep_wb <- loadWorkbook(rep_fp)
  rep_rawdata <- lapply(sheets, function(sh) readWorkbook(rep_wb, sheet = sh, colNames = FALSE))

  rep_last <- rep_rawdata[[length(rep_rawdata)]]
  rep_header_positions  <- seq(1, nrow(rep_last), by = 45)
  rep_header_candidates <- as.character(rep_last[rep_header_positions, 11])
  rep_header_stripped   <- stringr::str_sub(rep_header_candidates, 2, -2)
  rep_names_raw <- rep_header_stripped[!is.na(rep_header_stripped) & rep_header_stripped != ""]
  rep_names <- norm_area(rep_names_raw)
  rep_index_vec <- 0:(length(rep_names) - 1)

  # 実行（不一致が出たらユーザー選択で続行/中断）
  check_labels(rep_rawdata, spec, rep_index_vec, years, rep_names)
...
# 実行
if (!isTRUE(skip_label_check)) check_labels(sdata, spec, index_vec, years, NAMES_area)
...
# --------------------------------------------------
# 高齢化率など「割合データ」用の自動スケール
#   - df$Arate（%）に対して上限をデータから自動決定
#   - 5%刻み、%表示
# --------------------------------------------------
auto_rate_scale <- function(x, pad = 2, step = NULL, min_limit = NULL, max_limit = NULL, ...) {
  # 互換性維持:
  # - 旧: auto_rate_scale(x, pad=2) -> scale_y_continuous を返す
  # - 新: auto_rate_scale(x, step=..., pad=..., min_limit=..., max_limit=...) -> list(breaks, limits) を返す
  x <- x[is.finite(x)]

  # ---- list モード判定（step / min_limit / max_limit が指定されたら list を返す） ----
  list_mode <- !is.null(step) || !is.null(min_limit) || !is.null(max_limit)

  if (length(x) == 0) {
    if (list_mode) {
      return(list(breaks = seq(0, 100, by = ifelse(is.null(step), 5, step)),
                  limits = c(ifelse(is.null(min_limit), 0, min_limit),
                             ifelse(is.null(max_limit), 100, max_limit))))
    }
    return(scale_y_continuous(limits = c(0, 100), labels = function(v) paste0(v, "%")))
  }

  # 既定は「5刻み」
  by <- ifelse(is.null(step), 5, step)

  maxv <- ceiling(max(x) / by) * by
  maxv <- maxv + pad

  # 上限/下限の強制
  if (!is.null(min_limit)) maxv <- max(maxv, min_limit)
  if (!is.null(max_limit)) maxv <- min(maxv, max_limit)

  # 0未満は出さない（率は基本0以上）
  minv <- ifelse(is.null(min_limit), 0, min_limit)

  if (list_mode) {
    return(list(
      breaks = seq(minv, maxv, by = by),
      limits = c(minv, maxv)
    ))
  }
...
# ③ 年の数
analysis_period <- as.character(end_year)

# 最新年の高齢化率（名前付きベクトルに正規化：1列だけでも names を保持）
tmp_latest_arate <- mat_ageing[analysis_period, , drop = FALSE]
latest_Arate <- as.numeric(tmp_latest_arate[1, ])
names(latest_Arate) <- colnames(tmp_latest_arate)
...
# 先頭（通常は「地区全体」）を落とす。ただし1行しか無い場合は落とさない。
if (nrow(df) > 1) df <- df[-1, ]
df$fill_color <- ifelse(df$Arate > 50, "#AD000E", "#0B318F")
# 先頭の地区名を取得（=「三ヶ日地区」の代わり）
special_area <- if (!is.null(names(latest_Arate))) names(latest_Arate)[1] else NA_character_
...
latest_total <- mat_TOT[analysis_period, , drop = FALSE]
latest_total <- as.numeric(latest_total[1, ])
names(latest_total) <- colnames(mat_TOT)
