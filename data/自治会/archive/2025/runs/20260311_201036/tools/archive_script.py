...
def _find_project_root_by_data(start_dir: Path) -> Path:
    ...
    想定:
    - スクリプトは <project_root>/fetch/ 配下に置く
    - data は <project_root>/data/ に置く（fetch の1つ上）

    誤爆防止:
    - 以前の実行で <project_root>/fetch/data/ が出来ていても、原則として
      <project_root>/data/ を優先する。
    """

    start_dir = start_dir.resolve()

    # 典型: <root>/fetch/*.py で実行 → <root>/data を使う
    if start_dir.name.lower() == "fetch" and (start_dir.parent / "data").is_dir():
        return start_dir.parent

    # 万一 fetch/data と root/data の両方がある場合でも root を優先
    if start_dir.name.lower() == "fetch" and (start_dir / "data").is_dir() and (start_dir.parent / "data").is_dir():
        return start_dir.parent

    # それ以外: 上に向かって data がある階層を探す（最初に見つかった場所）
    for cur in [start_dir] + list(start_dir.parents):
        if (cur / "data").is_dir():
            # cur が fetch なら、一段上もチェックして上を優先
            if cur.name.lower() == "fetch" and (cur.parent / "data").is_dir():
                return cur.parent
            return cur

    # 見つからない場合は start_dir を返す（後段で mkdir される）
    return start_dir
